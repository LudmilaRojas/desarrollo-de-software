...
//    · POO     → class Cart, class GameCatalog, class AuthManager, class NexusDB
...
//  Tablas: users, sessions, recovery_tokens, library
//  Persistencia: localStorage (exporta DB como binario)
// ════════════════════════════════════════════
class NexusDB {
  constructor() {
    this.db = null;
  ...
  /**
   * Inicializa sql.js y crea/carga la base de datos.
   * Debe llamarse una sola vez al arrancar la app.
   */
  async init() {
    try {
      // sql.js se carga via CDN (ver index.html)
      const SQL = await initSqlJs({
        locateFile: file => `https://cdnjs.cloudflare.com/ajax/libs/sql.js/1.10.3/${file}`
      });

      // Intentar restaurar DB guardada en localStorage
      const saved = localStorage.getItem('nexus_db');
      if (saved) {
        try {
          const buf = Uint8Array.from(atob(saved), c => c.charCodeAt(0));
          this.db = new SQL.Database(buf);
          console.log('🗄️ Base de datos restaurada desde localStorage');
        } catch (e) {
          console.warn('No se pudo restaurar la DB, creando una nueva.', e);
          this.db = new SQL.Database();
        }
      } else {
        this.db = new SQL.Database();
        console.log('🗄️ Nueva base de datos SQLite creada');
      }
  ...
  /** Espera a que la DB esté lista (para llamadas antes del init) */
  onReady(fn) {
    if (this._ready) fn();
  ...
  _persist() {
    try {
      ...
      localStorage.setItem('nexus_db', btoa(binary));
    } catch (e) {
      console.error('Error al persistir la DB:', e);
    }
  }

  // ── USERS ────────────────────────────────

  /** Inserta un usuario nuevo. Lanza error si el email ya existe. */
  ...
  updatePassword(email, newHash) {
    this.db.run('UPDATE users SET password_hash = ? WHERE email = ?', [newHash, email]);
    this._persist();
  }
  ...
  insertToken({ token, email, expiresAt }) {
    this.db.run(
  ...
  getToken(token) {
    const stmt = this.db.prepare('SELECT * FROM recovery_tokens WHERE token = ?');
    stmt.bind([token]);
    if (stmt.step()) {
      const row = stmt.getAsObject();
      stmt.free();
      return { token: row.token, email: row.email, expiresAt: row.expires_at, usedAt: row.used_at };
    }
    stmt.free();
    return null;
  }
  ...
  markTokenUsed(token) {
    this.db.run('UPDATE recovery_tokens SET used_at = ? WHERE token = ?', [Date.now(), token]);
    this._persist();
  }

  /** Elimina tokens expirados */
  cleanExpiredTokens() {
    this.db.run('DELETE FROM recovery_tokens WHERE expires_at < ?', [Date.now()]);
    this._persist();
  }

  // ── LIBRARY ──────────────────────────────

  /** Devuelve array de game_ids del usuario */
  getLibrary(email) {
    const results = [];
    const stmt = this.db.prepare('SELECT game_id FROM library WHERE email = ?');
    stmt.bind([email]);
    while (stmt.step()) {
      results.push(stmt.getAsObject().game_id);
    }
    stmt.free();
    return results;
  }
  ...
  addToLibrary(email, gameId) {
    try {
      this.db.run(
        'INSERT OR IGNORE INTO library (email, game_id) VALUES (?, ?)',
        [email, gameId]
      );
      this._persist();
    } catch (e) {
      console.error('Error al agregar a biblioteca:', e);
    }
  }

  /** Elimina todos los juegos de la biblioteca de un usuario */
  clearLibrary(email) {
    this.db.run('DELETE FROM library WHERE email = ?', [email]);
    this._persist();
  }

  // ── DEBUG ─────────────────────────────────

  /** Muestra todas las tablas en consola (útil para debug) */
  debugDump() {
    console.group('🗄️ NexusDB — Dump completo');
    ['users', 'recovery_tokens', 'library'].forEach(table => {
      const rows = [];
      const stmt = this.db.prepare(`SELECT * FROM ${table}`);
      while (stmt.step()) rows.push(stmt.getAsObject());
      stmt.free();
      console.table(rows.length ? rows : [{ '(vacío)': true }]);
      console.log(`Tabla: ${table} — ${rows.length} fila(s)`);
    });
    console.groupEnd();
  }
}

/** Instancia global de la DB (se inicializa en DOMContentLoaded) */
const nexusDB = new NexusDB();
...
class AuthManager {
  ...
  // ── Persistencia via NexusDB (SQLite) ────

  _loadUsers() {
    // Los usuarios ahora viven en SQLite; this.users es un caché en memoria
    // Se popula bajo demanda via nexusDB.getUser()
    this.users = new Proxy({}, {
      get: (_, email) => nexusDB._ready ? nexusDB.getUser(email) : undefined,
      set: () => true // escrituras ignoradas: se hacen via nexusDB
    });
  }

  _saveUsers() {
    // No-op: nexusDB persiste automáticamente en cada operación
  }
  ...
  login({ email, password }) {
    ...
    const user = nexusDB.getUser(emailKey);
  ...
  recoverPassword(email) {
    ...
    nexusDB.cleanExpiredTokens();

    // Generar token de recuperación (24 caracteres)
    ...
    nexusDB.insertToken({ token, email: emailKey, expiresAt });
  ...
  validateRecoveryToken(token) {
    const recovery = nexusDB.getToken(token);
  ...
  resetPassword(token, newPassword) {
    ...
    if (!nexusDB.getUser(email)) {
      return { ok: false, error: 'La cuenta no existe.' };
    }

    // Actualizar contraseña en SQLite
    nexusDB.updatePassword(email, this._hash(newPassword));

    // Marcar token como usado
    nexusDB.markTokenUsed(token);
...
function handleLogout() {
  ...
  renderNavAuth();
  catalog.render();
  showToast(`👋 Hasta luego, ${username}! Tu biblioteca queda guardada en la base de datos.`);
}
...
function getUserLibrary() {
  ...
  const gameIds = nexusDB.getLibrary(auth.currentUser.email);
...
function addToLibrary(gameId) {
  ...
  nexusDB.addToLibrary(auth.currentUser.email, gameId);
}

/** No necesaria: SQLite persiste los datos automáticamente en localStorage */
function restoreLibraryBackup(email) {
  // Con SQLite la biblioteca persiste entre sesiones sin necesidad de backup manual
  return false;
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    await nexusDB.init();
    console.log('🗄️ NexusDB lista');
  } catch (err) {
    console.error('❌ No se pudo inicializar la base de datos:', err);
    // Mostrar aviso al usuario
    document.body.insertAdjacentHTML('afterbegin', `
      <div style="background:#ff3c6e;color:#fff;text-align:center;padding:.75rem;font-weight:600;position:fixed;top:0;left:0;right:0;z-index:9999;">
        ⚠️ Error al cargar la base de datos. Recargá la página.
      </div>
    `);
    return;
  }
  ...
  window.debugDB = () => nexusDB.debugDump();
  console.log('💡 Tip: ejecutá debugDB() en la consola para inspeccionar la base de datos SQLite.');
});
